...
- Runtime guard plugins `subagent-handoff-output-guard`, `tool-error-latch`, and `tool-request-guard` belong in this repository under `extensions/*`; do not maintain same-id copies under `~/.openclaw/extensions`.
- Deployment-local Feishu helper plugins remain in `~/.openclaw/extensions` and should be committed through the `openclaw-workspace` repo. This includes `feishu-file-outbox-router`, `feishu-researcher-export-mirror`, and `feishu-researcher-delegation-guard`.
- Keep `openclaw-integration/extensions` for generic, productized, or runtime-critical plugins that must pass source-tree build validation. Duplicating the same plugin id in bundled and local extension roots causes duplicate plugin warnings and uncertain override behavior.
