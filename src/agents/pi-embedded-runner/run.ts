...
function combineAbortSignals(signals: Array<AbortSignal | undefined>): AbortSignal | undefined {
  ...
  if (defined.some((signal) => signal.aborted)) {
    return defined.find((signal) => signal.aborted);
  }
...
function buildCancelledPendingRunResult(params: {
  sessionId: string;
  startedAt: number;
  provider?: string;
  model?: string;
    ...
    return await enqueueSession(async (): Promise<EmbeddedPiRunResult> => {
      ...
      const globalRunPromise = enqueueGlobal(async (): Promise<EmbeddedPiRunResult> => {
        ...
        const combinedAbortSignal = combineAbortSignals([
          params.abortSignal,
          getPendingEmbeddedRunAbortSignal(pendingRunToken),
        ]);
