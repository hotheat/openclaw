...
describe("model-selection", () => {
  ...
  describe("parseModelRef", () => {
    ...
    it("normalizes openai gpt-5.4 refs to openai-codex provider", () => {
      ...
      expect(parseModelRef("openai/gpt-5.4-pro", "anthropic")).toEqual({
        provider: "openai-codex",
        model: "gpt-5.4-pro",
      });
    });

    it("normalizes openai gpt-5.5 refs to openai-codex provider", () => {
      expect(parseModelRef("openai/gpt-5.5", "anthropic")).toEqual({
        provider: "openai-codex",
        model: "gpt-5.5",
      });
      expect(parseModelRef("gpt-5.5", "openai")).toEqual({
        provider: "openai-codex",
        model: "gpt-5.5",
      });
      expect(parseModelRef("openai/gpt-5.5-pro", "anthropic")).toEqual({
        provider: "openai-codex",
        model: "gpt-5.5-pro",
      });
    });
