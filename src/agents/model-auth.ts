...
export async function resolveApiKeyForProvider(params: {
  ...
  if (provider === "openai") {
    ...
    if (hasCodex) {
      throw new Error(
        'No API key found for provider "openai". You are authenticated with OpenAI Codex OAuth. Use openai/gpt-5.5 (routed to Codex OAuth) or set OPENAI_API_KEY to use the OpenAI API provider.',
      );
    }
  }
