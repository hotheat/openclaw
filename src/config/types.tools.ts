...
/**
 * @deprecated The artifact_jobs tool has been removed. Kept temporarily so older configs can
 * validate and be written back without activating any runtime behavior.
 */
...
export type ToolsConfig = {
  ...
  web?: {
    search?: {
      /** Enable Brave-backed web_search tool (default: true). */
      enabled?: boolean;
    ...
    grokSearch?: {
      /** Enable synthesized Grok search tool (default: true). */
      enabled?: boolean;
      /** xAI API key (optional; defaults to XAI_API_KEY env var). */
      apiKey?: string;
      /** Model to use (defaults to "grok-4-1-fast"). */
      model?: string;
      /** Return structured inline citations when available (default: false). */
      inlineCitations?: boolean;
      /** Timeout in seconds for Grok search requests. */
      timeoutSeconds?: number;
      /** Cache TTL in minutes for Grok search results (default: 0 = disabled). */
      cacheTtlMinutes?: number;
    };
    fetch?: {
      ...
      maxChars?: number;
      /** Hard cap for maxChars; when omitted, uses max(20000, configured maxChars). */
      maxCharsCap?: number;
  ...
  loopDetection?: ToolLoopDetectionConfig;
  /**
   * @deprecated The artifact_jobs tool has been removed. This value is accepted for config
