...
describe("runHeartbeatOnce – heartbeat model override", () => {
  ...
  it("passes heartbeatThinkingOverride from defaults heartbeat config", async () => {
    const replyOpts = await runDefaultsHeartbeat({ thinking: "off" });
    expect(replyOpts).toEqual(
      expect.objectContaining({
        isHeartbeat: true,
        heartbeatThinkingOverride: "off",
      }),
    );
  });
