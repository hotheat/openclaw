...
function resolveProviderFromContext(ctx: MsgContext, cfg: OpenClawConfig): ChannelId | undefined {
  ...
  if (
    explicitMessageChannel === INTERNAL_MESSAGE_CHANNEL ||
    explicitMessageChannel === CONTROL_UI_MESSAGE_CHANNEL ||
    explicitMessageChannel === WEBCHAT_MESSAGE_CHANNEL
  ) {
    return undefined;
  }
  const direct =
  ...
  for (const candidate of candidates) {
    ...
    if (
      normalizedCandidateChannel === INTERNAL_MESSAGE_CHANNEL ||
      normalizedCandidateChannel === CONTROL_UI_MESSAGE_CHANNEL ||
      normalizedCandidateChannel === WEBCHAT_MESSAGE_CHANNEL
    ) {
      return undefined;
    }
    const normalized =
