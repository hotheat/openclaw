(function () {
  ...
  const treeModel = window.createSessionTreeModel({
    entries,
    byId,
    labelMap,
  });
  ...
  function renderHeader() {
    ...
    if (warnings && warnings.length > 0) {
      html += `<div class="system-prompt" style="border-left: 3px solid var(--warning); color: var(--warning)">
            <div class="system-prompt-header">Warnings</div>
            <div class="system-prompt-full" style="display: block">${warnings
              .map((warning) => escapeHtml(warning.message || warning.code || String(warning)))
              .join("<br />")}</div>
          </div>`;
    }
  ...
  searchInput.addEventListener("input", (e) => {
    treeModel.setSearchQuery(e.target.value);
    forceTreeRerender();
  });
  ...
  document.querySelectorAll(".filter-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      treeModel.setFilterMode(btn.dataset.filter);
      forceTreeRerender();
    });
  });
