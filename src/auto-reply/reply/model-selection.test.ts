...
describe("createModelSelectionState resolveDefaultReasoningLevel", () => {
  it("defaults off even when catalog model has reasoning true", async () => {
  ...
  it("uses global reasoningDefault", async () => {
    const state = await createModelSelectionState({
      cfg: { agents: { defaults: { reasoningDefault: "on" } } } as OpenClawConfig,
      agentCfg: undefined,
  ...
  it("prefers per-agent reasoningDefault over global default", async () => {
    const cfg = {
      agents: {
        defaults: { reasoningDefault: "on" },
        list: [{ id: "researcher", reasoningDefault: "off" }],
      },
    } as OpenClawConfig;
    const state = await createModelSelectionState({
      cfg,
