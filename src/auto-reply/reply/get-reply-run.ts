...
import { buildInboundMediaNote, INBOUND_MEDIA_REPLY_HINT } from "../media-note.js";
...
export async function runPreparedReply(
  ...
  const mediaReplyHint = mediaNote ? INBOUND_MEDIA_REPLY_HINT : undefined;
  ...
  if (!resolvedThinkLevel && prefixedCommandBody) {
    ...
    if (
      maybeLevel &&
      (maybeLevel !== "xhigh" ||
        supportsXHighThinking(provider, model, modelState.allowedModelCatalog))
    ) {
  ...
  const followupRun = {
    ...
    run: {
      ...
      timeoutMs,
      timeoutOverrideSeconds: params.timeoutOverrideSeconds,
