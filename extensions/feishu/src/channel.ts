...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount> = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- When sending a local file, image, PDF, or other media in Feishu, you must call `message` with `action=send`, `channel=feishu`, and `filePath` (or `path`). If you are sending to the current conversation, you may omit `target`; use an explicit target when sending elsewhere or when you want to pin the destination.",
      "- Do not paste local filesystem paths such as `/tmp/...`, `workspace/...`, or `.outbox/...` into reply text. Send the file with the tool instead.",
