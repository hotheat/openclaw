...
- `AI Code Review`: run the automated review passes for pull requests
...
`codex_review` and `claudecode_review` are separate workflow jobs, so they can run in parallel with the `CI` workflow. Inside the `CI` workflow, `test` waits for both `lint` and `build`.
...
| `AI Code Review` | `codex_review`      | Codex automated PR review                               | workflow-managed |
| `AI Code Review` | `claudecode_review` | Claude Code automated PR review                         | workflow-managed |
