...
Experimental: enable via `tools.exec.applyPatch.enabled`. An omitted or empty `allowModels`
allows every provider/model; use full `provider/model` entries to narrow access.
