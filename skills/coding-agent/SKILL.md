...
Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.openclaw`, `$OPENCLAW_STATE_DIR`, or active OpenClaw state dirs.
...
- Worker must send completion or failure via `openclaw message send`.
...
- If worker fails or hangs, respawn or ask; do not silently hand-code instead.
...
1. Create or reuse a GitHub issue as durable spec.
...
- `list`: running or recent sessions.
- `poll`: status.
- `log`: output.
- `submit`: send input and Enter.
- `write`: raw stdin.
- `paste`: paste text.
- `kill`: terminate.
